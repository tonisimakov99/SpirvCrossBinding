...
namespace SpirvCrossBinding.Generator
{
    public class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            //parserOptions.AddDefines("SHADERC_SHAREDLIB");
            //parserOptions.AddDefines("SHADERC_IMPLEMENTATION");
            var parseResult = ClangParser.ParseSourceFiles(options.FilePaths, parserOptions);
            ...
            var classSyntax = SyntaxFactory.ClassDeclaration("Shaderc");
                ...
                SyntaxFactory.AttributeArgument(
                    SyntaxFactory.LiteralExpression(
                        SyntaxKind.StringLiteralExpression,
                            SyntaxFactory.Literal("libs/spirv-cross-c-shared.dll"))));
            attributesArgsList = attributesArgsList.Add(
            ...
            using (var fileWriter = new StreamWriter(File.OpenWrite($"{options.OutDir}/SpirvCross.cs")))
            {
        ...
        private static TypeSyntax GetTypeSyntax(ASTContext context, Type type)
        {
            ...
            var templateParameterType = type as TemplateParameterType;
            ...
            if (arrayType != null)
            {
                return SyntaxFactory.ArrayType(GetTypeSyntax(context, arrayType.Type));
            }

            if (templateParameterType != null)
            {
                return SyntaxFactory.ParseTypeName("some");
            }
            ...
            if (classes.ContainsKey(typeName))
            {
                ...
                using (var fileWriter = new StreamWriter(File.OpenWrite($"{options.OutDir}/{_class.Name}.cs")))
                {
                    ...
                    foreach (var field in _class.Fields)
                    {
                        ...
                        {
                            typeSyntax = GetTypeSyntax(context, new BuiltinType(PrimitiveType.Int));
                        }
                        else
                        ...
                        if (name == "internal" || name == "base" || name == "params")
                            name = "_" + name;
                        if (name == "")
                            name = "max_dual_source_draw_buffers_ext";
